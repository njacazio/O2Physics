...
#undef particleCase
    ...
    // NSigma
    std::string axisTitle = Form("N_{#sigma}^{TPC}(%s)", pT[id]);
    ...
    histos.add(hdelta_pt_pos[id].data(), axisTitle + " Positive", kTH2F, {ptAxis, deltaAxis});
    histos.add(hdelta_pt_neg[id].data(), axisTitle + " Negative", kTH2F, {ptAxis, deltaAxis});
