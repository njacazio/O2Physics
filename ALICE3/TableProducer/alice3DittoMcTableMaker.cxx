...
struct DittoEventDataProducer {
  ...
  Configurable<std::vector<int>> pdgsToStore{"pdgsToStore",
                                             {211, -211, 321, -321, 310, 2212, -2212},
                                             "PDGs of particles to store"};
  ...
  bool isSelected(Particle const& particle) const
  {
    ...
    if (std::find(pdgsToStore.value.begin(), pdgsToStore.value.end(), particle.pdgCode()) == pdgsToStore.value.end()) {
      return false;
    }
