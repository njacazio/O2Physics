...
/// \brief  Writer for the collection of LUTs for FlatTrackSmearer
...
void writeLUTCollection(std::string geometryFile = "/home/njacazio/alice/O2Physics/ALICE3/Macros/Configuration/a3geo.ini")
{
  ...
  { // Writing part
    o2::fastsim::FlatTrackSmearer mSmearer;
    mSmearer.SetName("ccdb_object");
    mSmearer.SetTitle("ccdb_object");
  ...
  { // Now we can get it back
    TFile inFile("/tmp/LUTCollection.root", "READ");
    o2::fastsim::FlatTrackSmearer* smearerFromFile = nullptr;
    inFile.GetObject("ccdb_object", smearerFromFile);
    if (smearerFromFile) {
      LOG(info) << "Successfully read LUT collection from file. Printing content:";
      smearerFromFile->print();
    } else {
      LOG(error) << "Failed to read LUT collection from file.";
    }
    inFile.Close();
  }
}
